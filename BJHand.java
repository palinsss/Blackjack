...
public class BJHand implements Hand {
    ...
    private boolean isBlackJack() {
        ...
        return cards.size() == 2 && cards.getFirst().getScore() + cards.getLast().getScore() == 21;
    }
    ...
    public int addCard(Card card) throws IllegalOperationException {
        // TODO: implementation
        if (this.getScore() > 21) {
            throw new IllegalOperationException("Impossible to add a card, because it's already busted!");
        }
        if (this.getScore() == 21) {
            throw new IllegalOperationException("Impossible to add a card, because it's already Blackjack!");
        }
